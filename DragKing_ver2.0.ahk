...
GroupAdd,  group_explorer, ahk_class AutoHotkeyGUI

GroupAdd,  group_explorer, EmEditor
GroupAdd,  group_explorer, XYplorer
GroupAdd,  group_explorer, xplorer2
GroupAdd,  group_explorer, Total Commander
GroupAdd,  group_explorer, Q-Dir
GroupAdd,  group_explorer, Excel
GroupAdd,  group_explorer, Vision4
GroupAdd,  group_explorer, MINGW64
;GroupAdd,  group_explorer, Microsoft Visual Studio
;GroupAdd,  group_explorer, Android Studio

specil_win_title := ["Android Studio", "Microsoft Visual Studio", "PetBadge", "项目"]

For k, v in specil_win_title
{
	;OutputDebug ktv_specil_win_title=%v% 
	GroupAdd,  group_specil, % v
}

#IfWinNotActive, ahk_group group_explorer
;--------------------------------------------------------------
...
~LButton Up::
valid_win := current_win_is_valid()
...
COPY:
;OutputDebug, ktv_Copy= =%valid_win%=
if valid_win = 0
{
	return
}
...
IfNotExist,%applicationname%.ini
{
  ...
  worddelimiters=spacetabnewlinecarridgereturn`,`;`%``:._\/()[]{}<>|�!"#�=?^~�*�'@�$��
...
;-------------------------------------------------------------- 
;	#IfWinActive ahk_group group_special
;	#IfWinActive 只对热键加程序一个整体有用,单独对程序没有用.
;	#IfWinActive
;--------------------------------------------------------------

#IfWinActive, ahk_group group_specil
	; Alt + 鼠标左键
		~!LButton Up::
		valid_win = 1
		;OutputDebug, ktv_WinActive= %valid_win%
		Gosub,COPY
		Return
...
current_win_is_valid()
{
	; 要在函数中引用现有的全局变量 (或创建新的), 需要在使用前声明此变量为全局的
	global specil_win_title
	...
	For k, v in specil_win_title
	{
		;OutputDebug ktv_current_win=%v% 

		IfInString, OutputVar, %v%
		{
			ret = 0
			break
		}
